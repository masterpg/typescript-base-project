declare module '@polymer/polymer/polymer-element' {
  ...
  interface AsyncModule {
    run(fn: () => any, timerId?: number): number;
    cancel(timerId: number): number;
  }

  class Debouncer {
    setConfig(asyncModule: AsyncModule, callback: () => any);
    cancel(): void;
    flush(): void;
    isActive(): boolean;
    static debounce(debouncer: Debouncer|any, asyncModule: AsyncModule, callback: () => any): Debouncer;
  }

  function enqueueDebouncer(debouncer: Debouncer);
  ...
  function mixinBehaviors(behaviors: (Object|Array<any>), klass: HTMLElement);
  ...
  class DomBind extends OptionalMutableData(PropertyEffects(HTMLElement)) {
    attributeChangedCallback();
    render();
  }

  class Element extends ElementMixin(HTMLElement) {
  }

  class DomRepeat extends OptionalMutableData(Element) {
    items: any[];
    as: string;
    indexAs: string;
    itemsIndexAs: string;
  ...
  class ArraySelector extends ArraySelectorMixin(Element) {
  }
